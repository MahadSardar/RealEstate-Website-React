  ...
  return (
    <div className='absolute top-0 left-0 w-full z-10'>
        <div className='container mx-auto flex justify-between items-center px-6 py-4 md:px-20 lg:px-32 bg-transparent'>
            ...
            {/*-------------Mobile-menu---------------  */}
            <div className={`md:hidden ${showMenu ? "fixed w-full":"h-0 w-0"} top-0 right-0 bottom-0 overflow-hidden bg-white
            transition-all`}>
              <div className='flex justify-end p-6 cursor-pointer'>
                <img src={assets.cross_icon} alt="" className='w-6' onClick={()=>{setShowMenu(false)}}/>
              </div>
              <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-bold'>
                ...
                <a onClick={()=>{setShowMenu(false)}} href="#Testmonials" className='px-4 py-2 rounded-full inline-block'>Testmonials</a>
              </ul>
            </div>
    </div>
  )
